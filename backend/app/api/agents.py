...
async def create_agent(
    ...
    # Template-declared MCP servers (e.g. shibui/finance for trading agents) are
    # installed asynchronously in the background task below — never synchronously
    # during the request — so creation is not blocked on an external Smithery call.
    # For trading agents this means shibui/finance lands in the agent's tool list
    # shortly after creation rather than relying on the agent to install it on first
    # use via the MCP_INSTALLER skill. Failures are logged and swallowed — agent
    # creation must never fail because an external Smithery call did.
