...
class SetVarStmt:
    ...
    expr: str          # may be `@outputs.<field>` for action chains
...
async def _is_tool_enabled_for_agent(
...
async def _exec_skill_via_gateway(
    ...
    while _asyncio.get_event_loop().time() < deadline:
        ...
        async with async_session() as db:
            r = await db.execute(
                select(GatewayMessage).where(GatewayMessage.id == msg_id)
            )
...
async def _exec_run_action(stmt: RunActionStmt, ctx: _ExecCtx) -> None:
    ...
    if ctx.agent_id is not None and target.startswith("tool://"):
        ...
        try:
            ...
            if not await _is_tool_enabled_for_agent(ctx.agent_id, tool_name):
                msg = (
        ...
        try:
            ...
            if not skill_found:
                ...
                skill_content = await storage.read_text(skill_key)
                info["skill_name"] = skill_name
...
def _get_active_topic(parsed: ParsedScript, state: ScriptState) -> ScriptTopic | None:
    ...
    # Default to start_agent if present, else first declared topic.
    if parsed.start_agent:
        return parsed.start_agent
    if parsed.topics:
        return next(iter(parsed.topics.values()))
    return None
...
# ════════════════════════════════════════════════════════════════════════
# 5.  State persistence  (storage runtime facade)
# ════════════════════════════════════════════════════════════════════════
...
async def save_state(agent_id: uuid.UUID, state: ScriptState, session_id: str = ""):
    ...
    await storage.write_text(key, json.dumps(data, ensure_ascii=False, indent=2))
