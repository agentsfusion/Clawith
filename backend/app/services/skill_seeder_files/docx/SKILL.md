---
name: DOCX
description: "Use this skill whenever creating, reading, editing, or manipulating Word documents (.docx files). Triggers: Word doc, word document, .docx, report, memo, letter, template, table of contents, headings, page numbers, tracked changes, comments, find-and-replace. Do NOT use for PDFs, spreadsheets, or general coding tasks."
---
...
- `pip install defusedxml` (XML parsing for editing)
...
**Landscape orientation:** docx-js swaps width/height internally:
```javascript
size: {
...
- **Never use tables as dividers/rules** - use `border: { bottom: { ... } }` on a Paragraph instead
